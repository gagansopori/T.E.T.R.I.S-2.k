...
public class TetrisObjects : MonoBehaviour {
	...
	void Update () {
        if(Input.GetKeyDown(KeyCode.LeftArrow))
        {
            ...
            if (IsValidGridPosition())
                ...
                transform.position += new Vector3(1, 0, 0);             //Check with x=0, if rotation quality is bad
        ...
        else if (Input.GetKeyDown(KeyCode.DownArrow) || Time.time - lastFall >= 1)
        {
            ...
            else
            {
                ...
                MatrixGrid.DestroyCompleteRow();

                FindObjectOfType<SpawnerScriptController>().SpawnRandom();
                enabled = false;
            }
